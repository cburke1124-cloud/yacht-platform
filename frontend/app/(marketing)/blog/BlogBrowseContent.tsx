...
interface BlogBrowseContentProps {
  initialPosts?: any[];
...
}: BlogBrowseContentProps) {
  const router = useRouter();
  const searchParams = useSearchParams();

  const [posts, setPosts] = useState<any[]>(initialPosts);
  const [recentPosts, setRecentPosts] = useState<any[]>(initialPosts.slice(0, 5));
...
export default function BlogBrowseContent(props: BlogBrowseContentProps) {
  return (
