...
function DetailPanel({
  ...
  const [stage, setStage] = useState<Stage>(inquiry.lead_stage);
  const [score, setScore] = useState(inquiry.lead_score);
...
export default function InquiriesPage() {
  ...
  const stageTotal = (stage: Stage) =>
    summary.find((s) => s.stage === stage)?.count ?? 0;
          ...
          <div className="bg-white border rounded-xl overflow-hidden shadow-sm">
            <table className="min-w-full divide-y divide-gray-100 text-sm">
              <thead className="bg-gray-50">
                <tr>
                  {["Contact", "Listing", "Stage", "Score", "Note", "Assigned To", "Date"].map(
                    (h) => (
                      <th
                        key={h}
                ...
                {inquiries.map((inq) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      <p className="font-medium text-gray-800">{inq.sender_name}</p>
                      <p className="text-xs text-gray-400">{inq.sender_email}</p>
                    </td>
                    ...
                    <td className="px-4 py-3 w-28">
                      <ScoreBar score={inq.lead_score} />
                    </td>
                    <td className="px-4 py-3 text-gray-500 max-w-xs truncate text-xs italic">
                      {inq.notes || "—"}
                    </td>
